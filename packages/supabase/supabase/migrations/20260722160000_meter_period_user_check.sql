-- Defense-in-depth: refuse to charge a credit period that does not belong to
-- the metering user (prevents cross-tenant debit if p_period_id is ever wrong).
...
begin
  ...
  if v_delta_in = 0 and v_delta_out = 0 then
    if cursor_row.cost_source = 'hosted' and p_period_id is not null then
      ...
      for update;
      if found then
        if period_row.user_id is distinct from p_user_id then
          raise exception 'credit period user mismatch';
        end if;
